...
/**
 * Daftar kejadian yang belum menunjukkan pelaksanaan selesai (bukan CLOSED/SELESAI), untuk tabel modal dashboard.
 * Logika selaras dengan KPI: invers dari filter CLOSE/SELESAI pada status_pelaksanaan_edukasi.
 */
final class ListPeerPressurePelaksanaanBelumKejadianAction
{
    ...
    public function __invoke(?int $year, ?int $month, int $page, int $perPage): array
    {
        ...
        $query = PeerPressureKejadianEdukasi::query()
            ->where(function ($q): void {
                $q->whereRaw(
                    'NOT (UPPER(TRIM(IFNULL(status_pelaksanaan_edukasi, \'\'))) LIKE ? OR UPPER(TRIM(IFNULL(status_pelaksanaan_edukasi, \'\'))) LIKE ?)',
                    ['%CLOSE%', '%SELESAI%']
                );
            });
