...
class DOPMController extends Controller
{
    ...
    /**
     * Hitung status matriks dari data yang sudah di-load (untuk batch/kalender).
     ...
     */
    public static function hitungStatusMatriksDariData(
        $ipkRow,
        $okkList,
        ?string $locationName,
        ?string $locationDetailName,
        ...
        $hasIpk = $ipkRow !== null;
        $durasiJam = $hasIpk ? ($ipkRow->durasi_jam ?? null) : null;
        $okkList = $okkList instanceof \Illuminate\Support\Collection ? $okkList : collect($okkList ?? []);
        $hasOkk = $okkList->count() > 0;

        $okkLayer1 = collect();
        $okkLayer2Up = collect();
        if ($hasOkk) {
            $namaLayer1N = $namaLayer1 ? trim(strtolower($namaLayer1)) : null;
            $namaLayer2N = $namaLayer2 ? trim(strtolower($namaLayer2)) : null;
            $namaLayer3N = $namaLayer3 ? trim(strtolower($namaLayer3)) : null;
            $namaLayer4N = $namaLayer4 ? trim(strtolower($namaLayer4)) : null;
            foreach ($okkList as $okk) {
                $namaPengawas = trim(strtolower($okk->nama_pengawas ?? ''));
                $layerPengawas = trim(strtolower($okk->layer_pengawas ?? ''));
                $isLayer1 = false;
                if ($namaLayer1N && $namaPengawas) {
                    if (strpos($namaPengawas, $namaLayer1N) !== false || ($layerPengawas && strpos($layerPengawas, '1') !== false)) {
                        $isLayer1 = true;
                    }
                }
                if ($isLayer1) {
                    ...
                    $isLayer2Up = false;
                    if ($namaPengawas) {
                        if ($namaLayer2N && (strpos($namaPengawas, $namaLayer2N) !== false || strpos($namaLayer2N, $namaPengawas) !== false)) {
                            $isLayer2Up = true;
                        }
                        if (!$isLayer2Up && $namaLayer3N && (strpos($namaPengawas, $namaLayer3N) !== false || strpos($namaLayer3N, $namaPengawas) !== false)) {
                            $isLayer2Up = true;
                        }
                        if (!$isLayer2Up && $namaLayer4N && (strpos($namaPengawas, $namaLayer4N) !== false || strpos($namaLayer4N, $namaPengawas) !== false)) {
                            $isLayer2Up = true;
                        }
                    }
                    if ($layerPengawas && (strpos($layerPengawas, '2') !== false || strpos($layerPengawas, '3') !== false || strpos($layerPengawas, '4') !== false)) {
                        $isLayer2Up = true;
                    }
                    if ($isLayer2Up) {
                        $okkLayer2Up->push($okk);
        ...
        if ($hasOkkLayer1 && $durasiJam !== null) {
            ...
            if (count($durasiParts) === 2) {
                ...
                if ($targetOkkCount > 0 && $okkLayer1Count >= $targetOkkCount) {
                    ...
                    $jarakMenit = $durasiRata >= 3 && $durasiRata <= 6 ? 30 : 60;
                    $isValidJarak = true;
                    for ($i = 1; $i < count($tsList); $i++) {
                        $diffMinutes = abs($tsList[$i]->diffInMinutes($tsList[$i - 1], false));
                        if ($diffMinutes < $jarakMenit) {
                            $isValidJarak = false;
                            break;
                        }
        ...
        if (!$hasIpk || !$hasOkk) {
            $alasan = [];
            if (!$hasIpk) {
                $alasan[] = 'Tidak ada IPK';
            }
            if (!$hasOkk) {
                $alasan[] = 'Tidak ada OKK';
            }
        ...
        if ($hasIpk && $hasOkkLayer1 && $isOkkSesuaiTarget && !$isOkkFraud && $hasOak) {
            if ($hasLayer2UpInIkk) {
                if ($hasOkkLayer2Up) {
                    return ['status' => 'Hijau', 'alasan' => 'Lengkap: IPK, OKK Layer 1 sesuai target, OKK Layer 2/3/4 ada, OAK ada'];
                }
            } else {
                return ['status' => 'Hijau', 'alasan' => 'Lengkap: IPK, OKK Layer 1 sesuai target, OAK ada'];
            }
        }
        if ($hasIpk && $hasOkkLayer1 && $isOkkSesuaiTarget && $isOkkFraud) {
            return ['status' => 'Kuning', 'alasan' => 'IPK + OKK sesuai target tetapi ada fraud (jarak/jumlah OKK Layer 1)'];
        }
        if ($hasIpk && $hasOkk && !$hasOak) {
            return ['status' => 'Kuning', 'alasan' => 'IPK dan OKK ada, tetapi belum ada OAK (DIC mitra maupun BC)'];
        }
        if ($hasIpk && $hasOkkLayer1 && $isOkkSesuaiTarget && !$isOkkFraud && $hasLayer2UpInIkk && !$hasOkkLayer2Up) {
            $layerNames = array_filter([$namaLayer2, $namaLayer3, $namaLayer4]);
            return ['status' => 'Kuning', 'alasan' => 'OKK Layer 1 sesuai target, belum ada OKK Layer 2/3/4 (' . implode(', ', $layerNames) . ')'];
        }
        $alasan = [];
        ...
        return ['status' => 'Kuning', 'alasan' => implode(', ', $alasan)];
    }
