<?php

declare(strict_types=1);

namespace App\Http\Requests\PilotProjectValidation;

use App\Models\PilotProjectValidationProject;
use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\Rule;

class PilotProjectValidationProjectUpdateRequest extends FormRequest
{
    public function authorize(): bool
    {
        return true;
    }

    /**
     * @return array<string, mixed>
     */
    public function rules(): array
    {
        $param = $this->route('project');
        $ignoreId = $param instanceof PilotProjectValidationProject ? $param->getKey() : $param;

        return [
            'project_name' => [
                'required',
                'string',
                'max:255',
                Rule::unique('pilot_project_validation_projects', 'project_name')->ignore($ignoreId),
            ],
            'subtitle' => ['nullable', 'string'],
            'pilot_area' => ['nullable', 'string', 'max:512'],
            'support' => ['nullable', 'string'],
            'current_phase' => ['nullable', 'string', 'max:255'],
            'progress' => ['nullable', 'integer', 'min:0', 'max:100'],
            'current_period' => ['nullable', 'string', 'max:255'],
            'next_milestone' => ['nullable', 'string'],
        ];
    }
}
