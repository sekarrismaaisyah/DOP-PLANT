@php
   $filters = $filters ?? ['filter_date' => '', 'site' => '', 'perusahaan' => '', 'automation_status' => '', 'q' => ''];
   $filterOptions = $filterOptions ?? ['dates' => collect(), 'sites' => collect(), 'perusahaan' => collect(), 'automationStatuses' => collect()];
   ...
   $statusLabel = collect($filterOptions['automationStatuses'])->firstWhere('value', $filters['automation_status'])['label'] ?? 'Semua Status';
...
@php
   $filterRoute = $filterRoute ?? 'auto-banned.banned-monitoring.index';
@endphp

<form method="GET" action="{{ route($filterRoute) }}" id="ab-daily-filter-form" class="flex flex-wrap items-center justify-end gap-2.5">
   @if(($filters['q'] ?? '') !== '')
   ...
   <div class="relative" data-ab-filter-wrap>
      <button type="button" data-ab-filter-toggle class="{{ $pickerBtnClass }}" aria-haspopup="listbox" aria-expanded="false">
         ...
         <span class="flex min-w-0 flex-1 flex-col items-start leading-tight">
            ...
            <span id="ab-date-label" class="truncate text-xs font-semibold text-on-surface">{{ $dateLabel }}</span>
         </span>
      ...
      <input type="hidden" name="filter_date" id="ab-filter-date" value="{{ $filters['filter_date'] }}"/>
   </div>
   ...
   <div class="relative" data-ab-filter-wrap>
      <button type="button" data-ab-filter-toggle class="{{ $pickerBtnClass }}" aria-haspopup="listbox" aria-expanded="false">
         <span class="material-symbols-outlined text-primary/80 text-lg shrink-0">smart_toy</span>
         <span class="flex min-w-0 flex-1 flex-col items-start leading-tight">
            <span class="text-[9px] font-semibold uppercase tracking-wider text-on-surface-variant/70">Status</span>
            <span id="ab-status-label" class="truncate text-xs font-semibold text-on-surface">{{ $statusLabel }}</span>
         </span>
         <span class="material-symbols-outlined text-on-surface-variant/50 text-lg shrink-0">expand_more</span>
      </button>
      <div class="{{ $dropdownClass }}" data-ab-filter-menu role="listbox">
         <button type="button" class="{{ $optionClass }} ab-filter-option" data-name="automation_status" data-value="" data-label="Semua Status">Semua Status</button>
         @foreach($filterOptions['automationStatuses'] as $status)
         <button type="button" class="{{ $optionClass }} ab-filter-option" data-name="automation_status" data-value="{{ $status['value'] }}" data-label="{{ $status['label'] }}">{{ $status['label'] }}</button>
         @endforeach
      </div>
      <input type="hidden" name="automation_status" id="ab-filter-status" value="{{ $filters['automation_status'] }}"/>
   </div>
</form>
...
<script>
(function () {
   var form = document.getElementById('ab-daily-filter-form');
   if (!form) return;

   var labelMap = {
      filter_date: 'ab-date-label',
      site: 'ab-site-label',
      perusahaan: 'ab-perusahaan-label',
      automation_status: 'ab-status-label'
   };
