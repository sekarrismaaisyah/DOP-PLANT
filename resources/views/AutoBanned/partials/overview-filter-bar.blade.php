@php
   $filters = $filters ?? ['filter_date' => '', 'site' => '', 'perusahaan' => '', 'q' => ''];
   $filterOptions = $filterOptions ?? ['dates' => collect(), 'sites' => collect(), 'perusahaan' => collect()];
...
<form method="GET" action="{{ route($filterRoute) }}" id="ab-overview-filter-form" class="flex flex-wrap items-center justify-end gap-2.5">
   @if(($filters['q'] ?? '') !== '')
   ...
   <div class="relative" data-ab-filter-wrap>
      <button type="button" data-ab-filter-toggle class="{{ $pickerBtnClass }}" aria-haspopup="listbox" aria-expanded="false">
         ...
         <span class="flex min-w-0 flex-1 flex-col items-start leading-tight">
            ...
            <span id="ab-overview-date-label" class="truncate text-xs font-semibold text-on-surface">{{ $dateLabel }}</span>
         </span>
   ...
   <div class="relative" data-ab-filter-wrap>
      <button type="button" data-ab-filter-toggle class="{{ $pickerBtnClass }}" aria-haspopup="listbox" aria-expanded="false">
         ...
         <span class="flex min-w-0 flex-1 flex-col items-start leading-tight">
            ...
            <span id="ab-overview-site-label" class="truncate text-xs font-semibold text-on-surface max-w-[8rem]">{{ $siteLabel }}</span>
         </span>
         <span class="material-symbols-outlined text-on-surface-variant/50 text-lg shrink-0">expand_more</span>
      </button>
      <div class="{{ $dropdownClass }}" data-ab-filter-menu role="listbox">
   ...
   <div class="relative" data-ab-filter-wrap>
      <button type="button" data-ab-filter-toggle class="{{ $pickerBtnClass }}" aria-haspopup="listbox" aria-expanded="false">
         ...
         <span class="flex min-w-0 flex-1 flex-col items-start leading-tight">
            ...
            <span id="ab-overview-perusahaan-label" class="truncate text-xs font-semibold text-on-surface max-w-[8rem]">{{ $perusahaanLabel }}</span>
         </span>
         <span class="material-symbols-outlined text-on-surface-variant/50 text-lg shrink-0">expand_more</span>
      </button>
      <div class="{{ $dropdownClass }}" data-ab-filter-menu role="listbox">
      ...
      <input type="hidden" name="perusahaan" value="{{ $filters['perusahaan'] }}"/>
   </div>
</form>
...
<script>
(function () {
   var form = document.getElementById('ab-overview-filter-form');
   if (!form) return;
   var labelMap = { filter_date: 'ab-overview-date-label', site: 'ab-overview-site-label', perusahaan: 'ab-overview-perusahaan-label' };
   ...
   form.querySelectorAll('[data-ab-filter-toggle]').forEach(function (btn) {
      btn.addEventListener('click', function (e) {
         ...
         closeAllMenus();
         if (menu && !isOpen) { menu.classList.remove('hidden'); btn.setAttribute('aria-expanded', 'true'); }
      });
   });
   form.querySelectorAll('.ab-filter-option').forEach(function (opt) {
      opt.addEventListener('click', function () {
         ...
         var labelEl = document.getElementById(labelMap[name]);
