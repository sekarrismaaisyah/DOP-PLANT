...
<html class="light" lang="id">
   <head>
      ...
      <title>@yield('title', 'Pembatasan LV') — PT.Berau Coal</title>
   ...
   <body class="bg-[#f0f2f5] font-body text-on-surface min-h-screen flex flex-col">
      @php
         $navActive = $navActive ?? 'overview';
         $programLabel = $programLabel ?? 'Pembatasan LV';
         $navItems = $navItems ?? [];
      @endphp
         ...
         <div class="mx-auto px-8 py-4 flex justify-between items-center">
            <div class="flex items-center gap-10">
               <div class="flex flex-col">
                  ...
                  <p class="text-on-surface-variant text-[10px] font-bold uppercase tracking-widest">{{ $programLabel }}</p>
               </div>
               @if(count($navItems) > 0 || View::hasSection('nav'))
               ...
               <nav class="hidden md:flex gap-8">
                  ...
                  @endphp
                  <a class="{{ $isActive ? 'text-[#3952bc] border-b-2 border-[#3952bc] pb-1 font-bold text-sm tracking-tight' : 'text-[#595c5e] hover:text-[#3952bc] font-semibold text-sm tracking-tight transition-colors' }}" href="{{ $href }}">{{ $item['label'] }}</a>
                  @endforeach
               </nav>
               @endif
               @endif
            ...
            <div class="flex items-center gap-6">
               ...
               <div class="relative group hidden xl:block">
                  <input class="bg-[#f5f7f9] border-none rounded-full px-4 py-2 text-sm focus:ring-2 focus:ring-primary w-80 transition-all shadow-inner" placeholder="Search records..." type="text" readonly disabled aria-hidden="true"/>
