<div id="plv-planning-lv-data-panel">
      ...
      <table class="w-full min-w-[900px] text-left">
         <thead class="border-b border-outline-variant/20 bg-[#f8fafc]">
            <tr>
               ...
               <th class="px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-on-surface-variant">No Unit</th>
               <th class="px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-on-surface-variant">Driver</th>
   ...
   <div id="plv-planning-lv-pagination" class="hidden flex-col gap-3 border-t border-outline-variant/20 px-6 py-4 sm:flex-row sm:items-center sm:justify-between flex">
      <p id="plv-planning-lv-info" class="text-xs font-medium text-on-surface-variant"></p>
      <div id="plv-planning-lv-pages" class="flex flex-wrap items-center gap-2"></div>
   </div>
</div>
...
<script>
(function () {
   var panel = document.getElementById('plv-planning-lv-data-panel');
   if (!panel) return;

   var tbody = document.getElementById('plv-planning-lv-tbody');
   var paginationWrap = document.getElementById('plv-planning-lv-pagination');
   var infoEl = document.getElementById('plv-planning-lv-info');
   var pagesEl = document.getElementById('plv-planning-lv-pages');
   var dataUrl = @json(route('pembatasan-lv.planning.lv.data'));
   var destroyUrlBase = @json(url('/pembatasan-lv/planning/lv'));
   ...
   function renderRows(rows, meta) {
      if (!rows.length) {
         tbody.innerHTML = '<tr><td colspan="9" class="px-6 py-12 text-center text-sm text-on-surface-variant">Belum ada data planning LV.</td></tr>';
      ...
      tbody.innerHTML = rows.map(function (row, i) {
         ...
         return '<tr class="hover:bg-[#f8fafc]">' +
            ...
            '<td class="px-4 py-3"><button type="button" class="rounded-lg p-2 text-error hover:bg-error/10" data-plv-plan-lv-delete="' + row.id + '" title="Hapus"><span class="material-symbols-outlined text-lg">delete</span></button></td>' +
         '</tr>';
      }).join('');
   ...
   tbody.addEventListener('click', function (e) {
      var btn = e.target.closest('[data-plv-plan-lv-delete]');
      if (!btn) return;
      var id = btn.getAttribute('data-plv-plan-lv-delete');
      if (!window.confirm('Hapus planning LV ini?')) return;
