...
<html class="light" lang="en">
   <head>
      ...
      <title>Safety Performance Review</title>
      ...
      <style>
         ...
         /* Warna matriks: didefinisikan di peer-pressure-edukasi/partials/operational-performance-matrix.blade.php */
      </style>
   </head>
   <body class="bg-[#f0f2f5] font-body text-on-surface min-h-screen flex flex-col">
         ...
         <div class=" mx-auto px-8 py-4 flex justify-between items-center">
            <div class="flex items-center gap-10">
               <div class="flex flex-col">
                  <h1 class="font-headline font-bold text-[#3952bc] text-xl tracking-tighter leading-tight">Safety Performance Review</h1>
                  <p class="text-on-surface-variant text-[10px] font-bold uppercase tracking-widest">Alignment Meeting</p>
               </div>
      ...
      <main class="flex-grow w-full  mx-auto p-8 space-y-8">
         @php
            require resource_path('views/alignment/dummy_state.php');
         @endphp
         <script>
         window.__alignmentDummy = true;
         window.__alignmentMocks = @json($alignmentPageMocks);
         (function () {
            var orig = window.fetch;
            window.fetch = function (input, init) {
               var url = typeof input === 'string' ? input : (input && input.url) || '';
               if (!window.__alignmentDummy || !window.__alignmentMocks) {
                  return orig.apply(this, arguments);
               }
               if (url.indexOf('peer-pressure-edukasi') === -1) {
                  return orig.apply(this, arguments);
               }
               ...
               if (url.indexOf('/kejadian/') !== -1 && url.indexOf('dashboard') === -1) {
                  return jsonResponse(window.__alignmentMocks.detail);
               }
               if (url.indexOf('weekly-trend') !== -1) {
                  return jsonResponse(window.__alignmentMocks.weekly);
               }
               if (url.indexOf('highlight-issue-recommendation') !== -1) {
                  return jsonResponse(window.__alignmentMocks.highlight);
               }
               if (url.indexOf('compliance-breakdown') !== -1) {
                  return jsonResponse(window.__alignmentMocks.compliance);
               }
               if (url.indexOf('pelanggar-profiling') !== -1) {
                  return jsonResponse(window.__alignmentMocks.profiling);
               }
               return orig.apply(this, arguments);
            };
         })();
         ...
         <div class="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 pb-6 border-b border-outline-variant/30">
            <div>
               <nav class="flex items-center gap-2 text-xs font-bold text-on-surface-variant uppercase mb-2">
                  ...
                  <span class="text-primary">Alignment Meeting | Review Week 15 Tahun 2026</span>
               </nav>
               <h2 class="font-headline font-extrabold text-4xl text-on-background tracking-tight">Safety Performance Review</h2>
               <p class="text-on-surface-variant font-medium mt-1">PT Berau Coal</p>
            </div>
         ...
         <!-- KPI Row -->
         @php
            $heg1Grid = $hazardEvalGrid ?? ['weeks' => [], 'rows' => []];
            $heg1Weeks = $heg1Grid['weeks'] ?? [];
            $heg1ChartPayload = ['weeks' => $heg1Weeks, 'lineColor' => '#3952bc', 'rows' => []];
            foreach ($heg1Grid['rows'] ?? [] as $heg1r) {
               $heg1ChartPayload['rows'][] = [
                  'trend' => array_values($heg1r['values'] ?? []),
                  'lineColor' => $heg1r['bar'] ?? '#3952bc',
               ];
            }
         @endphp
         @if (count($heg1ChartPayload['rows'] ?? []))
         <script type="application/json" id="heg1-line-charts-data">@json($heg1ChartPayload)</script>
         @endif
         <div class="grid grid-cols-1 items-start md:grid-cols-2 lg:grid-cols-12 gap-6">
         <div
            id="peer-kpi-hazard-eval-card-1"
            class="group relative md:col-span-2 lg:col-span-3 overflow-hidden rounded-3xl border border-slate-200/80 bg-white text-left shadow-[0_1px_0_rgba(0,0,0,0.04),0_20px_50px_-24px_rgba(15,23,42,0.25)] ring-1 ring-slate-900/[0.04]">
            <div class="pointer-events-none absolute -right-24 -top-24 h-48 w-48 rounded-full bg-gradient-to-br from-primary/[0.12] to-transparent blur-2xl transition-opacity group-hover:opacity-100"></div>
            <div class="pointer-events-none absolute -bottom-16 -left-16 h-40 w-40 rounded-full bg-gradient-to-tr from-secondary/[0.08] to-transparent blur-2xl"></div>
            <div class="relative flex flex-col gap-0.5 border-b border-slate-100/90 bg-gradient-to-b from-white to-slate-50/40 px-4 py-2.5 sm:px-5 sm:py-3">
               <div class="min-w-0">
                  <p class="font-headline text-center text-[11px] font-bold uppercase tracking-[0.18em] text-slate-500 sm:text-left">Hazards Evaluation</p>
                  <!-- <p class="mt-0.5 text-center text-[10px] leading-snug text-slate-500 sm:text-left">Per minggu</p> -->
               </div>
            </div>
            <div class="relative px-3 pb-2 pt-1.5 sm:px-4 sm:pb-3">
               <div class="flex flex-col gap-3 sm:gap-4">
                  @foreach ($hazardEvalGrid['rows'] as $rIdx => $row)
                  @php
                     $vals = $row['values'];
                     $maxV = max($vals) > 0 ? max($vals) : 1;
                     $dec = (int) ($row['decimals'] ?? 2);
                     $bar = $row['bar'];
                  @endphp
                  <div class="border-b border-slate-100/90 pb-3 last:border-b-0 last:pb-0">
                     <div class="mb-2 flex items-start gap-2">
                        <span class="mt-0.5 h-2 w-2 shrink-0 rounded-full shadow-sm ring-2 ring-white" style="background-color: {{ $bar }}"></span>
                        <p class="min-w-0 flex-1 text-[10px] font-semibold leading-snug text-slate-800 sm:text-[11px]">{{ $row['label'] }}</p>
                     </div>
                     <div class="peer-chart-scroll w-full overflow-x-auto">
                        <div class="peer-chart-scroll-inner w-max min-w-full px-1 sm:px-2">
                           <div class="relative h-32 sm:h-36">
                              <div class="peer-chart-bars relative z-10 flex h-full w-full items-stretch gap-1 sm:gap-2">
                                 ...
                                 @php
                                    ...
                                    $weekLabel = $hazardEvalGrid['weeks'][$vi] ?? ('W' . ($vi + 1));
                                    $tipHazard = ($weekLabel ?? '').' — '.$fmt;
                                 @endphp
                                 <div class="peer-chart-bar-col group relative flex h-full min-h-0 min-w-[2.25rem] flex-1 basis-0 flex-col justify-end rounded-t-lg border-x border-t border-outline-variant/10 bg-[#f8fafc]" title="{{ $tipHazard }}">
                                    <div class="relative w-full" style="height: {{ $barH }}%">
                                       <span class="absolute -top-5 left-1/2 z-10 -translate-x-1/2 whitespace-nowrap text-[9px] font-semibold tabular-nums text-on-surface sm:text-[10px]">{{ $fmt }}</span>
                           ...
                           <div class="peer-chart-axis-labels mt-2 flex w-full min-w-0 gap-1 sm:gap-2 text-[9px] font-bold uppercase tracking-wider text-on-surface-variant sm:text-[10px] sm:tracking-widest">
                              @foreach ($vals as $vi => $_)
                              <span class="peer-chart-axis-tick min-w-[2rem] flex-1 basis-0 text-center leading-tight">{{ $hazardEvalGrid['weeks'][$vi] ?? ('W'.($vi + 1)) }}</span>
                              @endforeach
                           </div>
                        </div>
                     </div>
                     <div class="mt-2 rounded-xl border border-slate-200/70 bg-gradient-to-b from-white to-slate-50/80 p-1 shadow-[inset_0_1px_0_rgba(255,255,255,1),0_2px_8px_-4px_rgba(15,23,42,0.1)] ring-1 ring-slate-100/90">
                        <p class="mb-0.5 px-1 text-[9px] font-semibold uppercase tracking-wide text-slate-400">Tren (line)</p>
                        <div class="relative h-[5.5rem] w-full min-h-[5.5rem] sm:h-28">
                           <canvas class="heg1-chart-canvas block h-full w-full max-h-28" data-heg1-row="{{ $loop->index }}" role="img" aria-label="Grafik garis tren mingguan"></canvas>
                        </div>
                     </div>
                  </div>
                  @endforeach
            ...
            <div class="relative flex flex-wrap items-center gap-2 border-t border-slate-100/90 bg-gradient-to-r from-slate-50/90 via-white to-slate-50/80 px-4 py-2 sm:px-5">
               <p class="text-[10px] font-medium leading-relaxed text-slate-500">
                  Jumlah agregat (kartu):
                  <span class="font-bold text-slate-800">{{ number_format($dvPreTotal) }}</span>
               </p>
            </div>
         </div>

         
         @include('peer-pressure-edukasi.partials.operational-performance-matrix', ['matrixShellClass' => 'md:col-span-2 lg:col-span-7'])
         ...
         <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div class="lg:col-span-4 flex flex-col gap-6">
               <div class="rounded-2xl border border-outline-variant/20 bg-white p-6 shadow-sm anchored-card">
                  ...
                  @php $recRow = collect($esRows)->firstWhere('key', 'recency'); $recD = $recRow ? ($recRow['recency_detail'] ?? null) : null; @endphp
         ...
         <div class="bg-white rounded-2xl anchored-card overflow-hidden">
            <div class="p-6 border-b border-outline-variant/20 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
               ...
               <div class="flex w-full flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-end">
                  <form id="peer-dashboard-search-form" method="get" action="#" class="flex w-full min-w-0 flex-1 flex-col gap-2 sm:max-w-md sm:flex-row sm:items-center">
                     ...
                     <div class="relative min-w-0 flex-1">
                        ...
                        <input type="search" name="q" value="" placeholder="Cari (demo — tidak memfilter data)" autocomplete="off" class="w-full rounded-xl border border-outline-variant/30 bg-[#f8fafc] py-2 pl-10 pr-3 text-sm text-on-surface outline-none transition-shadow placeholder:text-on-surface-variant/60 focus:border-primary/40 focus:ring-2 focus:ring-primary/15" aria-label="Cari data kejadian">
                     </div>
                     <div class="flex shrink-0 gap-2">
                        <button type="button" class="inline-flex items-center justify-center gap-1.5 rounded-xl border border-outline-variant/30 bg-white px-4 py-2 text-xs font-bold shadow-sm transition-colors hover:bg-surface-container-high">Cari</button>
                        <button type="button" class="inline-flex items-center justify-center rounded-xl px-3 py-2 text-xs font-bold text-on-surface-variant hover:bg-[#f1f5f9]">Reset</button>
                     </div>
                  </form>
                  <div class="flex flex-wrap items-center gap-3">
            ...
            <div class="overflow-x-auto">
               <table class="w-full text-sm text-left">
                  ...
                  <tbody class="divide-y divide-outline-variant/10">
                     <tr class="hover:bg-[#f8fafc] transition-colors cursor-pointer group js-peer-kejadian-row" data-kejadian-id="9001" role="button" tabindex="0">
                        <td class="px-8 py-5">
                           <div class="font-bold text-on-surface">10 Apr 2026 08:30</div>
                           <div class="text-[10px] text-on-surface-variant flex items-center gap-1 mt-0.5">
                              <span class="material-symbols-outlined text-[12px]" data-icon="location_on">location_on</span> Stockyard A
                           </div>
                           <span class="mt-2 inline-block bg-primary-container/20 text-primary text-[9px] px-2.5 py-0.5 rounded-full font-bold uppercase tracking-wider border border-primary/10">Pelanggaran PSPP</span>
                        </td>
                        <td class="px-8 py-5">
                           <div class="font-bold">S10001 | Budi Santoso</div>
                           <div class="text-xs text-on-surface-variant">Operasi / Penggalian</div>
                        </td>
                        <td class="px-8 py-5">
                           <div class="flex -space-x-2">
                              <div class="relative w-8 h-8 shrink-0 rounded-full border-2 border-white shadow-md overflow-hidden bg-surface-container-high">
                                 <div class="flex h-full w-full items-center justify-center text-[10px] font-bold bg-secondary-fixed">DL</div>
                              </div>
                           </div>
                           <div class="text-[10px] mt-2 font-bold text-on-surface-variant">Leader: HSE Officer</div>
                        </td>
                        <td class="px-8 py-5 font-bold text-xs text-on-surface">45m</td>
                        <td class="px-8 py-5">
                        ...
                        <td class="px-8 py-5">
                           <span class="inline-flex rounded-full px-2 py-0.5 text-[10px] font-bold bg-emerald-100 text-emerald-800">SELESAI</span>
                        </td>
                     </tr>
                     <tr class="hover:bg-[#f8fafc] transition-colors cursor-pointer group js-peer-kejadian-row" data-kejadian-id="9002" role="button" tabindex="0">
                        <td class="px-8 py-5">
                           <div class="font-bold text-on-surface">08 Apr 2026 14:00</div>
                           <div class="text-[10px] text-on-surface-variant flex items-center gap-1 mt-0.5">
                              <span class="material-symbols-outlined text-[12px]" data-icon="location_on">location_on</span> Pit South
                           </div>
                           <span class="mt-2 inline-block bg-secondary-container/20 text-secondary text-[9px] px-2.5 py-0.5 rounded-full font-bold uppercase tracking-wider border border-secondary/10">Golden rules</span>
                        </td>
                        <td class="px-8 py-5">
                           <div class="font-bold">S10002 | Andi Wijaya</div>
                           <div class="text-xs text-on-surface-variant">Maintenance / Mekanik</div>
                        </td>
                        <td class="px-8 py-5">
                           <div class="flex -space-x-2">
                              <div class="relative w-8 h-8 shrink-0 rounded-full border-2 border-white shadow-md overflow-hidden bg-surface-container-high">
                                 <div class="flex h-full w-full items-center justify-center text-[10px] font-bold bg-primary-fixed">RK</div>
                              </div>
                              <div class="relative w-8 h-8 shrink-0 rounded-full border-2 border-white shadow-md overflow-hidden bg-surface-container-high">
                                 <div class="flex h-full w-full items-center justify-center text-[10px] font-bold bg-tertiary-fixed">AM</div>
                              </div>
                           </div>
                           <div class="text-[10px] mt-2 font-bold text-on-surface-variant">Leader: Supervisor A</div>
                        </td>
                        <td class="px-8 py-5 font-bold text-xs text-on-surface">30m</td>
                        <td class="px-8 py-5">
                           <a href="#" onclick="event.stopPropagation()" class="text-primary hover:underline flex items-center gap-1 text-xs font-bold transition-all relative z-10">
                           <span class="material-symbols-outlined text-lg" data-icon="attach_file">attach_file</span> View Records
                           </a>
                        </td>
                        <td class="px-8 py-5">
                           <span class="inline-flex rounded-full px-2 py-0.5 text-[10px] font-bold bg-amber-100 text-amber-900">BERJALAN</span>
                        </td>
                     </tr>
                     <tr class="hover:bg-[#f8fafc] transition-colors cursor-pointer group js-peer-kejadian-row" data-kejadian-id="9003" role="button" tabindex="0">
                        <td class="px-8 py-5">
                           <div class="font-bold text-on-surface">05 Apr 2026 09:15</div>
                           <div class="text-[10px] text-on-surface-variant flex items-center gap-1 mt-0.5">
                              <span class="material-symbols-outlined text-[12px]" data-icon="location_on">location_on</span> Workshop
                           </div>
                           <span class="mt-2 inline-block bg-primary-container/20 text-primary text-[9px] px-2.5 py-0.5 rounded-full font-bold uppercase tracking-wider border border-primary/10">Insiden ringan</span>
                        </td>
                        <td class="px-8 py-5">
                           <div class="font-bold">S10003 | Citra Dewi</div>
                           <div class="text-xs text-on-surface-variant">HSE / Inspeksi</div>
                        </td>
                        <td class="px-8 py-5">
                           <div class="flex -space-x-2">
                              <div class="relative w-8 h-8 shrink-0 rounded-full border-2 border-white shadow-md overflow-hidden bg-surface-container-high">
                                 <div class="flex h-full w-full items-center justify-center text-[10px] font-bold bg-secondary-fixed">BS</div>
                              </div>
                           </div>
                           <div class="text-[10px] mt-2 font-bold text-on-surface-variant">Leader: HSE Lead</div>
                        </td>
                        <td class="px-8 py-5 font-bold text-xs text-on-surface">60m</td>
                        <td class="px-8 py-5">
                           <div class="text-error font-bold text-xs flex items-center gap-1">
                           <span class="material-symbols-outlined text-lg" data-icon="warning">warning</span> Missing Evidence
                           </div>
                        ...
                        <td class="px-8 py-5">
                           <span class="inline-flex rounded-full px-2 py-0.5 text-[10px] font-bold bg-emerald-100 text-emerald-800">CLOSED</span>
                        </td>
                     </tr>
                  </tbody>
               </table>
            ...
            <div class="p-6 bg-[#f8fafc] flex justify-between items-center border-t border-outline-variant/20">
               <p class="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest">Showing 1–3 of 3 entries (demo)</p>
         ...
         <section class="overflow-hidden rounded-2xl border border-outline-variant/15 bg-white anchored-card" aria-labelledby="peer-highlight-heading">
            <div class="flex flex-col gap-3 border-b border-outline-variant/20 bg-gradient-to-r from-primary/[0.06] to-secondary/[0.04] px-6 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-8">
               <div>
                  ...
                  <p class="mt-1 text-[11px] text-on-surface-variant">Narasi ringkas mengikuti alur metrik di halaman ini (KPI, trend, evaluasi, insight, tabel). <span class="font-semibold text-on-surface">Demo statis</span> — respons JSON disimulasikan di browser, tanpa agregat aktual atau AI.</p>
               </div>
      ...
      (function () {
        ...
        const detailUrlBase = '/peer-pressure-edukasi/kejadian';
        ...
        function renderPeerPressureSection(d) {
          ...
          ringkasan += ''
            + '<div class="flex flex-col gap-1 py-3"><span class="text-[11px] font-medium text-neutral-500">Site</span><span class="text-sm text-neutral-900">' + escapeHtml(dash(d.site)) + '</span></div>'
      ...
      <script>
      (function () {
        const weeklyTrendUrl = '/peer-pressure-edukasi/dashboard/weekly-trend';
        const peerHighlightUrl = '/peer-pressure-edukasi/dashboard/highlight-issue-recommendation';
        const complianceBreakdownUrl = '/peer-pressure-edukasi/dashboard/compliance-breakdown';
      ...
      <script>
      (function () {
        var profilingDetailUrl = '/peer-pressure-edukasi/dashboard/pelanggar-profiling';
      ...
      <script>
      (function () {
        function helcHexToRgb(hex) {
          var m = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex || '');
          return m
            ? { r: parseInt(m[1], 16), g: parseInt(m[2], 16), b: parseInt(m[3], 16) }
            : { r: 37, g: 99, b: 235 };
        }
        function helcAreaGradient(ctx, height, hex) {
          var rgb = helcHexToRgb(hex);
          var g = ctx.createLinearGradient(0, 0, 0, height || 100);
          g.addColorStop(0, 'rgba(' + rgb.r + ',' + rgb.g + ',' + rgb.b + ',0.22)');
          g.addColorStop(1, 'rgba(' + rgb.r + ',' + rgb.g + ',' + rgb.b + ',0.02)');
          return g;
        }
        function initHelcLineCharts() {
          var dataEl = document.getElementById('helc-line-charts-data');
          var card = document.getElementById('peer-kpi-hazard-eval-card-3');
          if (!dataEl || !card || typeof Chart === 'undefined') return;
          var payload;
          try {
          ...
          var defaultLineColor = payload.lineColor || '#4338ca';
          var rows = payload.rows || [];
          if (!weeks.length || !rows.length) return;

          window.__helcChartInstances = window.__helcChartInstances || [];
          window.__helcChartInstances.forEach(function (ch) {
            try {
              ch.destroy();
            } catch (err) {}
          });
          window.__helcChartInstances = [];

          var canvases = card.querySelectorAll('canvas.helc-chart-canvas');
          canvases.forEach(function (canvas, i) {
            var row = rows[i];
            if (!row || !row.trend || !row.trend.length) return;
            var ctx = canvas.getContext('2d');
            if (!ctx) return;
            var parent = canvas.parentElement;
            var h = parent ? parent.clientHeight : 112;
            var lineColor = row.lineColor || defaultLineColor;
            ...
            var chart = new Chart(ctx, {
                ...
                datasets: [
                  {
                    ...
                    backgroundColor: helcAreaGradient(ctx, h, lineColor),
                    fill: true,
                    tension: 0.35,
                    pointRadius: 4,
                    pointHoverRadius: 5,
              ...
              options: {
                ...
                layout: {
                  padding: { top: 20, right: 4, bottom: 2, left: 4 },
                },
                  ...
                  afterDatasetsDraw: function (chart) {
                    ...
                    chart.data.datasets.forEach(function (dataset, di) {
                      ...
                      meta.data.forEach(function (element, index) {
                        ...
                        var text = isNaN(num) ? String(raw) : num.toLocaleString('id-ID');
                        var x = element.x;
                        var y = element.y;
                        c.save();
                        c.font = '600 9px system-ui, "Segoe UI", sans-serif';
                        c.fillStyle = '#0f172a';
                        c.textAlign = 'center';
            ...
            window.__helcChartInstances.push(chart);
          });
        }
        function initHeg1LineCharts() {
          var dataEl = document.getElementById('heg1-line-charts-data');
          var card = document.getElementById('peer-kpi-hazard-eval-card-1');
          if (!dataEl || !card || typeof Chart === 'undefined') return;
          var payload;
          try {
            payload = JSON.parse(dataEl.textContent);
          ...
          var weeks = payload.weeks || [];
          var defaultLineColor = payload.lineColor || '#3952bc';
          var rows = payload.rows || [];
          if (!weeks.length || !rows.length) return;

          window.__heg1ChartInstances = window.__heg1ChartInstances || [];
          window.__heg1ChartInstances.forEach(function (ch) {
            try {
              ch.destroy();
            } catch (err) {}
          });
          window.__heg1ChartInstances = [];

          var canvases = card.querySelectorAll('canvas.heg1-chart-canvas');
          canvases.forEach(function (canvas, i) {
            var row = rows[i];
            if (!row || !row.trend || !row.trend.length) return;
            var ctx = canvas.getContext('2d');
            if (!ctx) return;
            var parent = canvas.parentElement;
            var h = parent ? parent.clientHeight : 88;
            var lineColor = row.lineColor || defaultLineColor;
            ...
            var chart = new Chart(ctx, {
              ...
              options: {
                ...
                maintainAspectRatio: false,
                layout: {
                  padding: { top: 16, right: 2, bottom: 0, left: 2 },
                },
                interaction: { intersect: false, mode: 'index' },
                plugins: {
                  legend: { display: false },
                  ...
                  afterDatasetsDraw: function (chart) {
                    ...
                    chart.data.datasets.forEach(function (dataset, di) {
                      ...
                      meta.data.forEach(function (element, index) {
                        ...
                        var y = element.y;
                        c.save();
                        c.font = '600 8px system-ui, "Segoe UI", sans-serif';
                        c.fillStyle = '#0f172a';
                        c.textAlign = 'center';
                        c.textBaseline = 'bottom';
            ...
            window.__heg1ChartInstances.push(chart);
          });
        }
        function runAlignmentLineCharts() {
          initHelcLineCharts();
          initHeg1LineCharts();
        }
        if (document.readyState === 'loading') {
          document.addEventListener('DOMContentLoaded', runAlignmentLineCharts);
        } else {
          runAlignmentLineCharts();
        }
      })();
      </script>
   </body>
