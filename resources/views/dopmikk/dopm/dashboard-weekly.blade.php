...
<style>
    ...
    /* Site list scroll untuk Status per Site Perusahaan */
    .site-list-scroll,
    .site-status-list-scroll {
        scrollbar-width: thin;
        scrollbar-color: rgba(0,0,0,0.12) transparent;
    }
    .site-list-scroll::-webkit-scrollbar,
    .site-status-list-scroll::-webkit-scrollbar {
        width: 4px;
    }
    .site-list-scroll::-webkit-scrollbar-track,
    .site-status-list-scroll::-webkit-scrollbar-track {
        background: transparent;
    }
    .site-list-scroll::-webkit-scrollbar-thumb,
    .site-status-list-scroll::-webkit-scrollbar-thumb {
        background: rgba(0,0,0,0.12);
        border-radius: 4px;
    }
    .site-list-scroll::-webkit-scrollbar-thumb:hover,
    .site-status-list-scroll::-webkit-scrollbar-thumb:hover {
        background: rgba(0,0,0,0.2);
    }
    .site-status-row:hover {
        border-color: rgba(0,0,0,0.08) !important;
        box-shadow: 0 2px 8px rgba(0,0,0,0.04);
    }
    .wh-32 { width: 32px; height: 32px; }
    .wh-42 { width: 42px; height: 42px; }
    
    /* Perusahaan accordion styles */
    .perusahaan-header:hover {
        filter: brightness(0.98);
    }
    .perusahaan-header[aria-expanded="true"] .accordion-chevron {
        transform: rotate(180deg);
    }
    .perusahaan-accordion .collapse {
...
<div class="hazard-detection-header">
    ...
    <div class="card-body py-3">
        <form method="get" action="{{ route('dopmikk.dopm.dashboard-weekly', ['week' => $filterWeek]) }}" id="dashboardFilterForm">
            <div class="row g-3 align-items-end">
                <div class="col-12 col-md-6 col-lg-4">
                    ...
                    <input type="hidden" id="filterWeek" value="{{ $filterWeek ?? '' }}" autocomplete="off">
                ...
                <div class="col-12 col-md-4 col-lg-4">
                    ...
                    <select name="site" id="filterSite" class="form-select rounded-3">
                        ...
                        @php
                            $staticSites = ['BMO 1', 'BMO 2', 'GMO', 'LMO', 'SMO', 'BMO 3','MARINE', 'HO'];
                        @endphp
                    ...
                    <div class="card mb-0 rounded-4 w-100">
                     <div class="card-body">
                       <div class="mb-2">
                         ...
                         <p class="mb-0 text-muted small">Total IKK Reschedule/Batal (week ini, distinct per kode)</p>
                       </div>
                       <div class="text-center py-3 mt-4">
                         <h5 class="mb-0 display-5 fw-bold">{{ number_format(count($rescheduleBatalCodes ?? [])) }} </h5>
                       </div>
                       <div class="text-center mt-3">
                   ...
                   <div class="card rounded-4 mb-0 w-100">
                     <div class="card-body">
                       <div class="d-flex align-items-start justify-content-between mb-3">
                          <div class="">
                            <h5 class="mb-0 fw-bold">Status per Perusahaan & Site</h5>
                          </div>
                          <div class="dropdown">
                    ...
                    @forelse($ikkMerah as $ikk)
                     @php
                        ...
                        $dopmJson = [
                            ...
                            'tanggal_dop' => $ikk->start_date ? \Carbon\Carbon::parse($ikk->start_date)->format('Y-m-d') : null,
                            'timestamp' => null,
                    ...
                    <div class="card-header bg-white border-bottom d-flex justify-content-between align-items-start">
                        <div>
                            ...
                            <small class="text-muted">
                                Data IKK Weekly (work permit) yang sudah di-approve KWTT, distinct per kode IKK. Klik [+] untuk detail IPK/OKK per tanggal.
                                @if(!empty($ikkDailyDetailsLazyLoad))
                                    <span class="d-block mt-1">Detail harian dimuat per baris saat dibuka (data banyak — mengurangi timeout).</span>
                                @else
                                    <span class="d-block mt-1">Detail harian sudah disiapkan di server.</span>
                                @endif
                            </small>
                        </div>
                        <div class="d-flex gap-2">
                            @if(count($ikkClickhouseListHarian ?? []) > 0)
                                ...
                                <a href="{{ route('dopmikk.dopm.dashboard-weekly.export-ikk-excel', array_filter(['week' => $filterWeek ?? now()->format('o-\\WW'), 'site' => request('site')], fn ($v) => $v !== null && $v !== '')) }}" 
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover align-middle mb-0 w-100" id="tableIkkClickhouseHarian">
                                   <thead class="table-light">
                                    ...
                                    <tbody>
                                        @php
                                            $rescheduleBatalCodesList = $rescheduleBatalCodes ?? [];
                                            $lazyDaily = !empty($ikkDailyDetailsLazyLoad ?? false);
                                        @endphp
...
(function() {
  ...
  var weeklyDashboardBase = @json(rtrim(url('/dopmikk/dopm/dashboard-weekly'), '/'));
  ...
  function isoMondayYmdFromWeekValue(weekValue) {
  ...
  function selectWeek(weekValue, startDisplay, endDisplay) {
    ...
    if (filterForm && weeklyDashboardBase) {
      filterForm.action = weeklyDashboardBase + '/' + encodeURIComponent(weekValue);
    }
    var fd = document.getElementById('filterDate');
    var monYmd = isoMondayYmdFromWeekValue(weekValue);
    if (fd && monYmd) fd.value = monYmd;
...
(function() {
    ...
    // Decode HTML entities untuk JSON parsing
    ...
    // Safe JSON parse dengan decode HTML entities
    function safeJsonParse(str) {
        try {
            var decoded = decodeHtmlEntities(str);
            return JSON.parse(decoded || '{}');
        } catch (e) {
            console.error('[DOPM Weekly] JSON parse error:', e, str);
            return {};
        }
    }
