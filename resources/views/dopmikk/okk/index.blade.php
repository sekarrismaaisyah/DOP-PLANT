...
@section('content')
    <x-page-title title="OKK" pagetitle="DOPM$IKK - OKK" />
        ...
        <div class="col-12 col-xl-6 d-flex">
            <div class="card rounded-4 w-100">
                <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                    ...
                    <a href="{{ route('dopmikk.okk.download-template') }}" class="btn btn-outline-primary btn-sm rounded-3">
                        <i class="material-icons-outlined me-1" style="font-size: 1rem;">download</i> Template Excel
                    </a>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('dopmikk.okk.import') }}" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3">
                            ...
                            <tbody>
                                @forelse ($entries as $entry)
                                    <tr>
                                        ...
                                        <td>{{ $entry->site ?? '-' }}</td>
                                        <td>{{ $entry->layer_pengawas ?? '-' }}</td>
                                        <td>
                                            <div class="d-flex gap-2">
                                                <a href="{{ route('dopmikk.okk.edit', $entry->id) }}" class="btn btn-sm btn-warning rounded-3">Edit</a>
                                                <form method="POST" action="{{ route('dopmikk.okk.destroy', $entry->id) }}" onsubmit="return confirm('Yakin hapus?');">
                                                    @csrf
                                                    @method('DELETE')
