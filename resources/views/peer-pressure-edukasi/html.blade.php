    ...
    tailwind.config = {
      theme: {
        extend: {
          ...
          boxShadow: {
            soft: "0 18px 45px rgba(31, 60, 42, 0.10)",
            popup: "0 24px 54px rgba(31, 60, 42, 0.18)"
          },
          borderRadius: {
            card: "24px"
          },
  ...
  <style type="text/tailwindcss">
    @layer base {
      ...
      * { @apply box-border; }
      ...
      body {
        @apply m-0 min-h-screen text-ink font-sans;
        background:
      ...
      button,
      select {
        @apply cursor-pointer rounded-full border border-line bg-white px-3.5 py-2.5 text-[13px] font-extrabold text-ink shadow-[0_8px_20px_rgba(31,60,42,0.06)] transition duration-200 ease-in-out;
      }
      ...
      th, td { @apply border-b border-[#edf2ee] px-3.5 py-3 text-left align-middle text-[13px]; }
      th { @apply whitespace-nowrap bg-[#f4f8f5] text-[11px] font-bold uppercase tracking-[.08em] text-[#4d5c54]; }
    ...
    @layer components {
      ...
      .hero { @apply relative overflow-hidden rounded-[30px] p-7 text-white shadow-soft; background: linear-gradient(135deg, #123e25 0%, #1e6b3c 52%, #49a24c 100%); }
      ...
      .eyebrow { @apply mb-3.5 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/15 px-3 py-2 text-xs font-extrabold uppercase tracking-[.12em] backdrop-blur; }
      .dot { @apply h-2 w-2 rounded-full bg-[#ffd85c] shadow-[0_0_0_5px_rgba(255,216,92,0.15)]; }
      h1 { @apply m-0 text-[clamp(28px,4vw,48px)] font-black leading-[1.02] tracking-[-.04em]; }
      .subtitle { @apply mt-3 max-w-[760px] text-[15px] leading-[1.55] text-white/85; }

      .hero-kpis { @apply grid grid-cols-2 gap-3; }
      .hero-kpi { @apply rounded-[18px] border border-white/20 bg-white/10 p-4 backdrop-blur-md; }
      .hero-kpi .label { @apply mb-2 text-xs text-white/75; }
      .hero-kpi .value { @apply text-[26px] font-black tracking-[-.03em]; }
      .hero-kpi .note { @apply mt-1 text-[11px] text-white/70; }
      ...
      .page-intro h2 { @apply m-0 mb-1.5 text-[22px] tracking-[-.03em]; }
      ...
      .card { @apply rounded-card border border-line/90 bg-white/85 p-5 shadow-soft backdrop-blur-md; }
      ...
      .card-title h2 { @apply text-[22px]; }
      .card-title h3 { @apply text-lg; }

      .muted { @apply text-muted; }
      .small { @apply text-xs; }
      .strong { @apply font-black; }

      .badge { @apply inline-flex items-center gap-1.5 whitespace-nowrap rounded-full px-2.5 py-[7px] text-[11px] font-black; }
      ...
      .score-card { @apply relative min-h-[186px] cursor-pointer overflow-hidden transition duration-200 ease-in-out; }
      .score-card:hover { @apply -translate-y-[3px]; }
      .score-card.selected { outline: 3px solid rgba(51,168,82,.28); }
      .score-card::after { content: ""; @apply absolute -bottom-[75px] -right-[50px] h-[170px] w-[170px] rounded-full bg-[#33a852]/10; }
      .score-top { @apply relative z-[2] flex items-start justify-between gap-2.5; }
      .site-name { @apply m-0 text-[26px] font-black tracking-[-.05em]; }
      .risk-score { @apply mt-[18px] text-[34px] font-black tracking-[-.06em]; }
      ...
      .bar-bg { @apply h-2 overflow-hidden rounded-full bg-[#eef3ef]; }
      .bar-fill { @apply h-full rounded-full bg-[#33a852]; }
      ...
      .rank { @apply grid h-9 w-9 place-items-center rounded-xl bg-[#f0f4f1] font-black; }
      .table-wrap { @apply overflow-auto rounded-[18px] border border-line bg-white; }
      ...
      .legend-item { @apply inline-flex items-center gap-[7px] text-xs font-bold text-muted; }
      ...
      .driver-detail-title { @apply mb-2 flex items-center justify-between gap-2.5 font-black; }
      ...
      .detail-chip { @apply inline-flex items-center rounded-full border border-line bg-[#f3f7f4] px-2 py-1 text-[10px] font-black text-[#496058]; }

      .bridge { @apply grid grid-cols-2 gap-3; }
      .bridge-box { @apply min-h-[150px] rounded-[18px] border border-line bg-white p-4; }
      .bridge-box h4 { @apply m-0 mb-2 text-[15px]; }
      ...
      .stat-tile .num { @apply mt-1.5 text-[26px] font-black tracking-[-.04em]; }
      ...
      .help-dot { @apply ml-1.5 grid h-4 w-4 place-items-center rounded-full border border-line bg-[#eef3ef] text-[10px] font-black text-muted; }
