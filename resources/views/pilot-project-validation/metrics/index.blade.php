...
<div class="bg-white rounded-2xl anchored-card overflow-hidden">
  <div class="p-6 border-b border-outline-variant/20 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
    <div><h2 class="font-headline font-bold text-xl text-on-surface">Metrics</h2><p class="text-xs text-on-surface-variant font-medium">Kelola tabel <code>pilot_project_validation_metrics</code>.</p></div>
    <a href="{{ route('pilot-project-validation.metrics.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-xs font-bold text-white shadow-md"><span class="material-symbols-outlined text-base">add</span> Tambah metric</a>
  </div>
  ...
  <div class="px-6 pt-4">
    <div class="rounded-xl border border-outline-variant/20 bg-[#f8fafc] p-4">
      <div class="mb-2 text-xs font-bold uppercase tracking-wide text-on-surface-variant">Import Excel Metrics</div>
      <p class="mb-3 text-[11px] text-on-surface-variant">
        Gunakan kolom: <strong>Project</strong>, <strong>Gate</strong>, <strong>Gate Title</strong>, <strong>Metric Name</strong>, <strong>Metric Description</strong>, <strong>Metric Type</strong>, <strong>Direction</strong>, <strong>Critical</strong>, <strong>Unit</strong>, <strong>Current Value</strong>, <strong>Pass Threshold</strong>, <strong>Conditional Threshold</strong>, <strong>Min</strong>, <strong>Max</strong>, <strong>Step</strong>, <strong>PIC Current Finding</strong>, <strong>PIC Evidence / Source</strong>, <strong>PIC Comment</strong>, <strong>Metric Status</strong>.
      </p>
      <form method="post" action="{{ route('pilot-project-validation.metrics.import-excel') }}" enctype="multipart/form-data" class="flex flex-wrap items-center gap-2">
        @csrf
        ...
        <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-xs font-bold text-white shadow-md"><span class="material-symbols-outlined text-base">upload_file</span> Upload Metrics Excel</button>
        <a href="{{ route('pilot-project-validation.metrics.template-excel') }}" class="inline-flex items-center gap-2 rounded-xl border border-outline-variant/30 bg-white px-4 py-2 text-xs font-bold shadow-sm hover:bg-surface-container-high"><span class="material-symbols-outlined text-base">download</span> Unduh contoh template</a>
      </form>
    </div>
  </div>
    ...
    <tbody class="divide-y divide-outline-variant/10">
      @php
        $groupedRows = $rows->getCollection()->groupBy(function ($item) {
          return $item->gate?->project?->project_name ?: 'Tanpa Project';
        });
      @endphp
      @forelse($groupedRows as $projectName => $projectRows)
        ...
        @foreach($projectRows as $row)
          <tr class="hover:bg-[#f8fafc] transition-colors align-top">
            ...
            <td class="px-8 py-5 text-xs">{{ $row->gate?->project?->project_name }}</td>
            <td class="px-8 py-5 text-xs">{{ $row->gate?->gate_label }}</td>
